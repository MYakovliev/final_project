...
public class ToSellerLots implements ActionCommand {
    ...
    private static final String COMMAND_TO_PAGING = "to_seller_lots";
    ...
    public CommandResult execute(HttpServletRequest request) {
        ...
        try {
            ...
            int amount = amountService.findLotBySellerIdAmount(user.getId());
            logger.debug("active lots: {}", amount);
            int pageAmount = (amount - 1 + AMOUNT_PER_PAGE) / AMOUNT_PER_PAGE;
            List<Lot> lots = service.findLotBySellerId(user.getId(), lotPageNumber, AMOUNT_PER_PAGE);
